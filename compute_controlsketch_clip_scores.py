...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=int,
        default=32,
        help="Accepted for compatibility; each score is computed from a 2-image input/sketch batch.",
    )
    parser.add_argument(
        "--jit",
        action="store_true",
...
def parse_num_strokes(run_name: str) -> int | None:
            ...
            errors.append(
                {
                    "category": category,
                    "sketch_image": as_json_path(sketch_path),
                    "error": f"No matching input image named {category}.* under input root.",
                }
            )
            continue
